...
def user_data(path):
    ...
    types = {'phone_no_m': 'str', 'city_name': 'str', 'county_name': 'str', 'idcard_cnt': 'float32',
             ...
             'call_dur_240+_ratio': 'float32', 'active_days_per_month': 'float32',
             'monthly_city_switch_count': 'float32', 'monthly_county_switch_count': 'float32', 'imei_count': 'float32',
             'opposite_no_m_count': 'float32', 'calltype_1_percentage': 'float32', 'calltype_2_percentage': 'float32',
    ...
    num_list = ['idcard_cnt', 'risk', 'monthly_call_count',
        ...
        'monthly_county_switch_count', 'imei_count', 'opposite_no_m_count', 'calltype_1_percentage',
        'calltype_2_percentage', 'broadcast_percentage', 'app_times'
    ]
    df.dropna(subset=['phone_no_m'], inplace=True)
    df.loc[:, str_list] = df[str_list].replace('nan', '', inplace=False)
    df.replace([np.inf, -np.inf], np.nan, inplace=True)
    avg = df[num_list].mean()
    df.loc[:, num_list] = df[num_list].fillna(value=avg)
    ...
    city_name_df = pd.DataFrame(city_name_np, columns=new_columns, dtype=np.float16, index=df.index)

    county_name_np = np.array(df['county_name'].tolist())
    new_columns = ['county_name' + str(i) for i in range(county_name_np.shape[1])]
    county_name_df = pd.DataFrame(county_name_np, columns=new_columns, dtype=np.float16,index=df.index)
